        ...
        with st.spinner('Analyse en cours... 🧠'):
            ...
            if eeg.shape[1] != 14:
                ...
                def compute_fft(signal, fs=FS):
                    ...
                    idx = np.where(freqs >= 0)  # garder seulement fréquences positives
                ...
                fig_f3_raw = px.line(y=f3_raw, labels={"index": "Temps (échantillons)", "value": "Amplitude (μV)"})
                st.plotly_chart(fig_f3_raw, use_container_width=True)

                st.subheader("Canal F3 Filtré ")
                fig_f3_filt = px.line(y=f3_filt, labels={"index": "Temps (échantillons)", "value": "Amplitude (μV)"})
                st.plotly_chart(fig_f3_filt, use_container_width=True)

                st.subheader("Canal F4 Brut")
                fig_f4_raw = px.line(y=f4_raw, labels={"index": "Temps (échantillons)", "value": "Amplitude (μV)"})
                st.plotly_chart(fig_f4_raw, use_container_width=True)

                st.subheader("Canal F4 Filtré ")
                fig_f4_filt = px.line(y=f4_filt, labels={"index": "Temps (échantillons)", "value": "Amplitude (μV)"})
                st.plotly_chart(fig_f4_filt, use_container_width=True)

# === GRAPHIQUES FFT ===
                st.subheader("Spectre de Fréquence - Alpha F3")
                fig_fft_f3 = px.line(x=freqs_f3, y=fft_f3, labels={"x": "Fréquence (Hz)", "y": "Amplitude"})
                fig_fft_f3.update_layout(title="FFT du signal Alpha - Canal F3 (8-12 Hz)")
                ...
                fig_fft_f4 = px.line(x=freqs_f4, y=fft_f4, labels={"x": "Fréquence (Hz)", "y": "Amplitude"})
                fig_fft_f4.update_layout(title="FFT du signal Alpha - Canal F4 (8-12 Hz)")
                fig_fft_f4.update_xaxes(range=[7, 13])
                st.plotly_chart(fig_fft_f4, use_container_width=True)
 
                # Comparaison Puissance Alpha
    ...
    if os.path.exists(RESULTS_FILE):
        ...
        fig1 = px.pie(df, names='prediction', title="Répartition des Prédictions")
        st.plotly_chart(fig1, use_container_width=True)

        st.subheader("Distribution de confiance LIKE")
        fig2 = px.histogram(df, x='confidence_like', nbins=20, title="Distribution de Confiance LIKE (%)")
